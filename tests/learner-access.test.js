...
test('les modules video expliquent clairement quoi filmer et quand valider', () => {
  const startModule = CORE_MODULES.find((module) => module.id === 'module-0');
  const finalModule = CORE_MODULES.find((module) => module.id === 'module-12');

  assert.match(startModule.actionLabel, /Filmer ma vidéo de départ de 3 à 5 minutes/);
  assert.match(startModule.videoBrief.title, /Vidéo à enregistrer/);
  assert.match(JSON.stringify(startModule.videoBrief.items), /réunion|présentation de projet|pitch/);
  assert.match(startModule.validationConfirmation, /enregistré ma vidéo de départ/);

  assert.match(finalModule.videoBrief.intro, /format court est un entraînement/);
  assert.match(JSON.stringify(finalModule.videoBrief.items), /présentation longue|atelier|conférence/);
  assert.match(finalModule.validationConfirmation, /enregistré ma vidéo finale/);
});
