...
<header class="sales-header">
  <nav class="sales-nav" aria-label="Navigation commerciale">
    ...
    <div>
      ...
      <a href="#faq">FAQ</a>
      <a href="${escapeHtml(product.loginUrl)}">Connexion</a>
  ...
  <section class="sales-hero" aria-labelledby="sales-title">
    ...
    <p class="intro">Level Up vous guide pas à pas pour préparer vos interventions avec une méthode reproductible. Les vidéos courtes de 3 à 5 minutes servent de repères d’entraînement ; la méthode vous aide ensuite à structurer aussi une réunion, une présentation longue, un atelier ou une conférence.</p>
...
<main class="sales-main">
  <section class="sales-section" aria-labelledby="problem-title">
    ...
    <p>La méthode ne promet pas de tout transformer. Elle vous aide à préparer concrètement une prise de parole claire et mieux structurée, qu’elle dure quelques minutes ou qu’elle serve de base à une intervention plus longue.</p>
  </section>
  ...
  <section id="difference" class="sales-section contrast-section" aria-labelledby="difference-title">
    ...
    <p>Cette formule ne remplace pas un accompagnement. Elle ne propose pas d’analyse personnalisée de votre posture, de votre voix, de vos blocages ou de votre vidéo. Elle vous donne un cadre solide pour préparer seul une intervention claire, des premiers entraînements courts jusqu’à des formats plus ambitieux.</p>
...
<main class="sales-main purchase-main">
  <section class="sales-section price-section" aria-labelledby="purchase-title">
    ...
    <p class="microcopy">Votre accès est personnel. La méthode ne remplace pas un accompagnement individuel et personnalisé, mais elle vous donne une structure concrète pour avancer seul, des premiers entraînements courts jusqu’à des prises de parole plus ambitieuses.</p>
  </section>
</main>`);
}
...
<main class="auth-shell">
  <section class="auth-panel" aria-labelledby="denied-title">
    ...
    <a class="button-link" href="${escapeHtml(product.loginUrl)}">Revenir à la connexion</a>
  </section>
</main>`);
}
...
function renderVideoBrief(videoBrief) {
  if (!videoBrief || !Array.isArray(videoBrief.items) || videoBrief.items.length === 0) {
    return '';
  }

  return `<div class="video-brief">
    <h3>${escapeHtml(videoBrief.title || 'Vidéo à enregistrer')}</h3>
    ${videoBrief.intro ? `<p>${escapeHtml(videoBrief.intro)}</p>` : ''}
    <ul class="brief-list">
      ${videoBrief.items.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}
    </ul>
  </div>`;
}

function renderActionBrief(selectedModule) {
  const criteria = Array.isArray(selectedModule.observableCriteria)
    ? selectedModule.observableCriteria
    : [];
  const hasWorksheet = selectedModule.worksheet && Array.isArray(selectedModule.worksheet.sections)
    && selectedModule.worksheet.sections.length > 0;

  return `<section class="action-brief" aria-labelledby="action-brief-title">
    <p class="section-label">À faire maintenant</p>
...
<main class="accompaniment-shell">
  ...
  <section class="accompaniment-card" aria-labelledby="video-share-title">
    ...
    <p>Filmez une séquence de 3 à 5 minutes issue de votre intervention préparée, puis partagez-la via un service externe : lien non listé YouTube, Google Drive, Dropbox, WeTransfer ou autre solution habituelle. Ce court extrait suffit pour recevoir un retour structuré, même si votre intervention complète est plus longue.</p>
    <ol>
...
<main class="completion-shell">
  <section class="completion-hero" aria-labelledby="completion-title">
    ...
    <p>Vous n’avez pas cherché à devenir parfait. Vous avez préparé, testé, observé et clarifié une prise de parole avec des vidéos courtes comme repères, puis construit une méthode réutilisable pour des formats plus longs.</p>
  ...
  <section class="completion-card" aria-labelledby="next-title">
    ...
    <div class="next-step-grid">
      ...
      <article>
        ...
        <p>Gardez le format court pour vous entraîner, puis augmentez progressivement la durée, l’enjeu et la taille du public.</p>
      </article>
      <article>
...
<main class="module-shell">
  ...
  ${renderActionBrief(selectedModule)}

  <section class="module-template" aria-labelledby="template-title">
    <h2 id="template-title">Comprendre avant de faire</h2>
    <div class="template-grid">
      <article>
        <p class="section-label">À comprendre</p>
        <ul>
